...
public class CustomUserDetails implements UserDetails {
    ...
    // 감싸고 있는 원본 엔티티, 아래 메서드들은 전부 이 엔티티의 값을 security 표준 형태로 변환해 전달한다.
    private User user;
    ...
    // DaoAuthenticationProvider가 passwordEncoder.matches(사용자 입력, 이 값)으로 대조한다.
    @Override
    public @Nullable String getPassword() {
        return user.getPassword();
    }

    // security가 말하는 username = "사용자를 식별하는 문자열"
    // loadUserByUsername의 username으로 값이 들어온다.
    @Override
    ...
    // 비밀번호가 만료되지 않았는가 (주기적으로 변경 강제 정책 등)
    @Override
